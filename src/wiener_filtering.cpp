...
wiener_filtering::wiener_filtering(surface_reconstruction * reca, field *fi, survey *su, dblarray shearamap, dblarray shearbmap, dblarray ps_array, dblarray noise_cov)
{
	surv = su;
	f = fi;
	rec = reca;
	...
	// store shear values to compute noise covariance
	//shear0.reserve(impix);
	//shear1.reserve(impix);
	wiener_residuals.reserve(impix);
	kwiener.reserve(impix);
	for (int i=0; i < impix; i++) {
		kwiener.push_back(.0);
		wiener_residuals.push_back({.0,.0});
	}
	...
	// read the power spectrum image instead
	// dblarray ps_array1;
	// fits_read_dblarr("../data/toy/psmap2d_shifted.fits", ps_array1);
	// std::vector<double> ps2d;
	// ps2d.reserve(npix*npix);
	// for (int i = 0; i < npix * npix; i++)
	// 	ps2d.push_back(ps_array1(i));

	// write Px to a fits file
	// double * Pximg1 = (double *) malloc(sizeof(double) * impix);
	// for (int i = 0; i < impix; i++)
	// 	Pximg1[i] = ps2d[i];
	// dblarray exptPx;
	// exptPx.alloc(Pximg1, npix, npix);
	// fits_write_dblarr("Px.fits", exptPx);
	...
	// read the covariance matrix instead
	// dblarray sn_array;
	// fits_read_dblarr("../data/toy/Sn.fits", sn_array);
	...
	for (int i=0; i < impix; i++) {
		// counter.push_back(0);
		Sn.push_back(noise_cov(i)); // Sn.push_back(.0); // push_back(sn_array(i));
		Px.push_back(shimg[i].real() ); //Px.push_back(ps_arr1(i)); // push_back(ps_array1(i));
		shear_res.push_back({shearamap(i), -shearbmap(i)});
	}

	std::cout << "arrived wiener filtering constructor" << '\n';
...
wiener_filtering::~wiener_filtering()
{
	...
	fftw_free(fft_frameHf1);
	fftw_free(fft_frameHb0);
	fftw_free(fft_frameHb1);
}

/*
 *  computes the diagonal noise covariance matrix of the binned shear
 */
 /*
void wiener_filtering::compute_noise_cov(std::vector<int> bin_ind)
{
	...
	int Niter = 200; // # of random realizations
	...
	for (int j = 0; j < impix; j++)
		if (Sn[j]==0)
			Sn[j] = 1e19;
		 	...
		 	Sn[j] = Sn[j] / (Niter - 1); // .125 / counter[j]

		std::cout << "Covariance Sn computed" << endl;
		for (int j = 0; j < 200; j++)
			std::cout << Sn[j] << " ";
		std::cout << std::endl;
}
...
void wiener_filtering::main_iteration()
{
	...
	// read the binned shear instead for validation purposes
	// dblarray g1_array;
	// fits_read_dblarr("../data/toy/g1.fits", g1_array);
	// dblarray g2_array;
	// fits_read_dblarr("../data/toy/g2.fits", g2_array);
	// for (int i = 0; i < npix * npix; i++)
	// 	shear_res.push_back({g1_array(i), -g2_array(i)});
  ...
  // fits_write_dblarr("kglimpse.fits", expt1); // this is the same as in glimpse output
	...
	// std::cout << "Inside main_iteration, print shear" << std::endl;
	// for (int i = 0; i < 100; i++)
	// 	std::cout << " " << shear_res[i].real();
	// std::cout << std::endl;
	//
	// std::cout << "Inside main_iteration, print shear" << std::endl;
	// for (int i = 0; i < 100; i++)
	// 	std::cout << " " << shear_res[i].imag();
	// std::cout << std::endl;

	// run wiener filtering core for the binned shear residuals
	kwiener = run_wiener_core(100);

	// std::cout << "Inside main, print kwiener" << std::endl;
	// for (int i = 0; i < 100; i++)
	// 	std::cout << " " << kwiener[i];
	// std::cout << std::endl;
  ...
  // fits_write_dblarr("glpluswf.fits", expt2);
	...
	// compute the wiener residuals as a last step
...
std::vector<std::complex <double>> wiener_filtering::compute_wiener_residuals(){
	...
	t = H_operator_new(t); // changed H_operator to H_operator_new
	std::transform(shear_res.begin(), shear_res.end(), t.begin(), t.begin(), std::minus<std::complex<double>>()); // y - H * xg

	std::complex <double> tmp = {0.,0.};
	for (int i = 0; i < impix; i++)
		t[i] =  Sn[i] > 1e2 ? tmp : t[i];

	// for (int i = 0; i < impix; i++)
	// 	t[i] = t[i] * Sn[i] ;

	// not working yet
	// write wiener residuals to a fits file
	double * img1 = (double *) malloc(sizeof(double) * impix);
	for (int i = 0; i < impix; i++)
		img1[i] = t[i].real();
	dblarray expta;
	expta.alloc(img1, npix, npix);
	fits_write_dblarr("wiener_residual_shear_a.fits", expta);

	double * img2 = (double *) malloc(sizeof(double) * impix);
	for (int i = 0; i < impix; i++)
		img2[i] = t[i].imag();
	dblarray exptb;
	exptb.alloc(img2, npix, npix);
	fits_write_dblarr("wiener_residual_shear_b.fits", exptb);


	return t;
}

std::vector<double> wiener_filtering::run_wiener_core(unsigned int Niter)
{
	...
	// std::cout << "Inside core, print Sn" << std::endl;
	// for (int i = 0; i < 1000; i++)
	// 	std::cout << " " << Sn[i];
	// std::cout << std::endl;
	//
	// std::cout << "Inside core, print Px" << std::endl;
	// for (int i = 0; i < 1000; i++)
	// 	std::cout << " " << Px[i];
	// std::cout << std::endl;
	...
	// std::cout << "Inside core, print Wfc" << std::endl;
	// for (int i = 0; i < 10; i++)
	// 	std::cout << " " << Wfc[i];
	// std::cout << std::endl;
	// std::cout << "Inside core, print Esn" << std::endl;
	// for (int i = 0; i < 10; i++)
	// 	std::cout << " " << Esn[i];
	// std::cout << std::endl;
	// std::cout << "Inside core, print data" << std::endl;
	// for (int i = 0; i < 10; i++)
	...
	*/
	...
	// // store Esn in a fits file to plot in notebook
	// dblarray tsnexp;
	// tsnexp.alloc(&Esn[0], npix, npix);
	// fits_write_dblarr("Esn.fits", tsnexp);
	...
	for (int iter = 0; iter < Niter; ++iter)
	{
		...
		// t equation
		t = H_operator_new(xg); // H * xg
		std::transform(shear_res.begin(), shear_res.end(), t.begin(), t.begin(), std::minus<std::complex<double>>()); // y - H * xg
		std::transform(Esn.begin(), Esn.end(), t.begin(), t.begin(), std::multiplies<std::complex<double>>()); // (eta / Sn) * (y - H * xg)

		// std::cout << "Before adjoint, print t" << std::endl;
		// for (int i = 0; i < 10; i++)
		// 	std::cout << " " << t[i].real();
		// std::cout << std::endl;
		t = H_adjoint_operator_new(t); // H^T * ( (eta / Sn) * (y-H*xg) )

		// std::cout << "After adjoint, print t" << std::endl;
		// for (int i = 0; i < 10; i++)
		// 	std::cout << " " << t[i].real();
		// std::cout << std::endl;
		...
		*/
		...
		// if (iter == 0){
		// 	std::cout << "Inside loop, print xg" << std::endl;
		// 	for (int i = 0; i < 10; i++)
		// 		std::cout << " " << xg[i].imag();
		// 	std::cout << std::endl;
		// }
...
std::vector<std::complex<double>> wiener_filtering::H_adjoint_operator(std::vector<std::complex<double>> gamma)
{
	...
	for (int y = 0; y < npix ; y++) {
		...
		// k2 = (y - npix ) * freqFactor; // this comes from testing in c++

		// int ky  = (y < npix / 2 ? y + npix / 2 : y - npix / 2);
		for (int x = 0; x < npix ; x++) {
			// k1 = (x - npix / 2) * freqFactor; //
			k1 = (x < npix / 2 ? x * freqFactor : (x - npix) * freqFactor); // this comes from python fft
			// k1 = (x - npix ) * freqFactor; // this comes from testing in c++
...
std::vector<std::complex<double>> wiener_filtering::H_operator_new(std::vector<std::complex<double>> kappa)
{
	std::vector<std::complex<double>> result(impix, {.0, .0});
...
std::vector<std::complex<double>> wiener_filtering::H_adjoint_operator_new(std::vector<std::complex<double>> gamma)
{
	std::vector<std::complex<double>> result(impix, {.0, .0});
	double freqFactor = 1. / ((double) npix); // 2.0 * M_PI / ((double) npix ) ; // 1.0 / ((double) npix); // / pixel_size
...
/*
for (int i=0; i < impix; i++) {
	shear0.push_back(shear_res[i].real());
	shear1.push_back(shear_res[i].imag());
}
*/
...
for (unsigned i=0; i < ngal; i++) {
	...
	*/
	...
	// for (int ind = 0; ind < impix ; ind++){
	// 	rwshear1[ind] = (double) (real(t[ind]));
	// 	rwshear2[ind] = (double) (imag(t[ind]));
	// }
