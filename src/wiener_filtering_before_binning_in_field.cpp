...
wiener_filtering::wiener_filtering(surface_reconstruction * reca, field *fi, survey *su, dblarray ps_array)
{
	...
	// read true kappa spectrum Px from file
	//dblarray ps_arr1;
	...
	std::vector<double> ps1d(ps_array.nx(),.0);
	for (int i = 0; i < ps_array.nx(); i++)
		ps1d[i] = ps_array(i);
	std::vector<double> ps2d = powerspec2d(ps1d);

	// fftshift the resulting image
	...
	*/
	...
	for (int i=0; i < impix; i++) {
		counter.push_back(0);
		Sn.push_back(0);
		Px.push_back(shimg[i].real()); //Px.push_back(ps_arr1(i)); // need to estimate this //
	}
...
void wiener_filtering::compute_noise_cov(std::vector<int> bin_ind)
{
	...
	//printf("Inside compute_noise_cov \n");
	int Niter = 100; // # of random realizations

	// get the shear values
	std::vector<double> rsh0(ngal,.0);
	std::vector<double> rsh1(ngal,.0);
	for (int i=0; i < ngal; i++) {
		rsh0[i] = surv->get_gamma1(i);
		rsh1[i] = surv->get_gamma2(i);
	}

	std::vector<std::complex<double>> noise_res(impix, .0);
	for (int i=0; i < Niter; i++) {
		// permute shear values
		std::random_shuffle(rsh0.begin(), rsh0.end());
		std::random_shuffle(rsh1.begin(), rsh1.end());

		// bin the permuted shear vectors, utilizing the same binning index
		noise_res = bincount(bin_ind, rsh0, rsh1);
...
void wiener_filtering::main_iteration()
{
  ...
  fits_write_dblarr("kglimpse.fits", expt1); // this is the same as in glimpse output
	...
	std::vector<double> kwiener = rwc(shear_res, 300);
...
std::vector<double> wiener_filtering::powerspec2d(std::vector<double> spectrum){
	...
	int * ni = (int *) malloc( npix * sizeof(int));
	for (int i = 0; i < npix; i++)
		ni[i] = int(i - npix / 2.);
...
std::vector<double> wiener_filtering::rwc(std::vector<std::complex <double>> data, int Niter)
{
	...
	double * ppo = (double *) malloc(sizeof(double) * 182);
	double * ppi = (double *) malloc(sizeof(double) * 182);
	double * ppe = (double *) malloc(sizeof(double) * 182);
...
void wiener_filtering::get_residuals()
{
	...
	f->comp_residuals(rec->get_kappa(), res1, res2);
...
std::vector<std::complex<double>> wiener_filtering::bincount(std::vector<int> ind1d, std::vector<double> residual1, std::vector<double> residual2)
{
	...
	for (unsigned i=0; i < vl; i++) {
		summ1[ind1d[i]] += residual1[i];
		summ2[ind1d[i]] += residual2[i];
	}
